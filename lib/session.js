...
function startExpirationTimer(session) {
    var existingTimer = expirationTable[session.id];
    if (existingTimer)
        clearTimeout(existingTimer);

    expirationTable[session.id] = setTimeout(function() {
        session.end();
        delete sessionTable[session.id];
    }, config.sessionExpirationTime * 1000);
}
