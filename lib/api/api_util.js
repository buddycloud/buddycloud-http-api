...
exports.discoverChannelNode = function(req, res, channel, subnode, callback) {
    ...
    var nodeId = '/user/' + channel + '/' + subnode;

    var server = discoverCache[domain];
    if (server) {
    ...
    discoverChannelServer(req, res, domain, function(server) {
        if (!server) {
            ...
            addToDiscoverCache(domain, server);
            callback(server, nodeId);
        }
    });
}

function addToDiscoverCache(domain, server) {
    discoverCache[domain] = server;
    setTimeout(function() {
        delete discoverCache[domain];
    }, config.discoveryExpirationTime * 1000);
}
